    ...
    <div className="test-library">
      ...
      {selectedTests.size > 0 && (
        <div className="library-bulkbar">
          ...
          <button type="button" className="library-bulk-btn danger" onClick={handleDeleteSelected}>
            🗑 Delete
          </button>
          <button
            type="button"
      ...
      {(() => {
        ...
        return groups.map((suite) => {
          ...
          const isOpen = filtering ? !filterCollapsed.has(suiteKey) : openSuites.has(suiteKey)
          return (
              ...
              {!isOpen ? null : tests.length === 0 ? (
                ...
                <ul className="library-list">
                  {tests.map((test) => {
                    ...
                    const allRuns =
                      test.runs && test.runs.length ? test.runs : test.lastRun ? [test.lastRun] : []
                      ...
                      <li key={test.fileName} className="library-item">
                        <div className="library-item-head">
                          ...
                          >
                            ...
                            {flaky.tag !== 'untested' && (
                              <span className={`flaky-tag ${flaky.tag}`} title={flaky.title}>
                                {flaky.label}
                              </span>
                            )}
                            ...
                            {failedRuns.map((run, ri) => (
                              <div key={ri} className="run-fail-entry">
                                <div className="run-fail-when">
                                  ...
                                  {run.failedAt !== undefined ? ` · step ${run.failedAt + 1}` : ''}
                                </div>
