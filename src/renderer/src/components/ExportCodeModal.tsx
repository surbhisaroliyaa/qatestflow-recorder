      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {exportEnvWarning.length > 0 && (
          <div className="edge-warn edge-warn-block">
            ...
            {exportEnvWarning.length === 1 ? 'is also an' : 'are also'} operating-system environment
            variable{exportEnvWarning.length === 1 ? '' : 's'} — on Windows <code>USERNAME</code> is
            your login name. Read directly, the test would silently use that instead of your value.
            This export reads <code>QA_{exportEnvWarning[0]}</code> instead and fails fast if it
            isn&rsquo;t set. To remove the guard, rename the token to something app-specific (e.g.{' '}
            ...
            <span className="saved-path">
              ...
              {savedPageOverwritten && (
                <span className="saved-path-warn">
                  ⚠ A different page class already existed at that path and was replaced. The class
                  name comes from the TEST name, so another test called “
                  {testName || 'recorded flow'}” shares this file — its spec now imports a class
                  that no longer matches it. Rename one of the tests and re-export.
                </span>
              )}
            </span>
          )}
