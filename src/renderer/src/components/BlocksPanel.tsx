          ...
          <>
            ...
            {blocks.length > 0 && (
              <div className="block-hint">
                🔗 linked — stays in sync when you edit the block · ⧉ copy — an independent snapshot
                you can edit here
              </div>
            )}
            {blocks.length === 0 ? (
              ...
              <ul className="block-list">
                {blocks.map((b) => {
                  ...
                  const linkNames = links
                    .map(
                      (l) =>
                        `• ${l.name}${l.suite ? ` (${l.suite})` : ''}${l.count > 1 ? ` ×${l.count}` : ''}`
                    )
                    .join('\n')
                  return (
                    <li key={b.fileName} className="block-row">
                      ...
                      >
                        🔗 {b.name}{' '}
                        <span className="block-count">{plural(b.stepCount, 'step')}</span>
                      </button>
