...
- Round-trip principle: a test is stored in a canonical internal model; Playwright/YAML/JSON are _views_ generated from it. Re-import of YAML/JSON restores the editable test.
...
**Resilient selector generation** — for each captured element, compute a _ranked_ set of candidate selectors and store them all, preferring in order:
...
_(Detailed schema, IPC design, and the canonical step model are out of scope for this product-focused PRD and belong in a technical design doc — flagged in §11.)_
