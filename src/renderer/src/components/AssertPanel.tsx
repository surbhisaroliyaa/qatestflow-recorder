    ...
    <div className="assert-panel" ref={checkPanelRef}>
      ...
      {pickedElement.unreliable && (
        <div className="pick-warning">
          ⚠ This element has no stable hooks (no id / role / text) — a check on it cannot replay
          reliably. Pick a more specific element instead (its label, or a container with an id).
        </div>
      )}
      ...
      {!pickedElement.unreliable && textCheckIsCircular(pickedElement.candidates, assertKind) && (
        <div className="pick-warning">
          ⚠ This element is found <em>by</em> its text, so a text check can only fail when the
          element is missing — it never really checks the wording.{' '}
          <button type="button" className="link-btn" onClick={() => handleChooseKind('visible')}>
            Use “Visible”
          </button>{' '}
      ...
      <div className="assert-kinds">
        {ASSERT_KINDS.filter(
          (kind) =>
            (kind !== 'checked' && kind !== 'unchecked') || pickedElement.checked !== undefined
        ).map((kind) => (
          <button
            key={kind}
            type="button"
