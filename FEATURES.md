...
<!-- counts:start -->
**Automated tests: 1013 unit tests** (42 files, `npm test`) and **111 browser tests** (6 files, `npm run test:dom`) — including a gate that generates specs with both exporters and runs them in real Playwright. _These numbers are generated by `node tools/doc-counts.mjs`; CI fails if they drift._
<!-- counts:end -->
<!-- prettier-ignore-end -->
