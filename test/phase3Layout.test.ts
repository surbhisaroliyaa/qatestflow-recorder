...
describe('a failed page load, in plain words', () => {
  it('explains the failures a tester actually meets', () => {
    ...
    expect(explainLoadError({ code: -102, description: 'ERR_CONNECTION_REFUSED' })).toMatch(/Nothing is answering/)
    expect(explainLoadError({ code: -105, description: 'ERR_NAME_NOT_RESOLVED' })).toMatch(/can’t be found/)
    expect(explainLoadError({ code: -106, description: 'ERR_INTERNET_DISCONNECTED' })).toMatch(/offline/)
    // Found by driving the real app: port 9 is one browsers refuse outright.
    expect(explainLoadError({ code: -312, description: 'ERR_UNSAFE_PORT' })).toMatch(/block this port/)
  })

  it('covers every certificate error with one sentence', () => {
    expect(explainLoadError({ code: -202, description: 'ERR_CERT_AUTHORITY_INVALID' })).toMatch(/certificate/)
    expect(explainLoadError({ code: -201, description: 'ERR_CERT_DATE_INVALID' })).toMatch(/certificate/)
  })

  it('still says something honest for a code it has no sentence for', () => {
    expect(explainLoadError({ code: -999, description: 'ERR_SOMETHING_NEW' })).toBe('The page couldn’t be loaded.')
  })
