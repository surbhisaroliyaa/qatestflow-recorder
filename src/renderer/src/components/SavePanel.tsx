    ...
    <div className="assert-panel">
      ...
      <div className="session-block">
        ...
        <p className="tag-note">
          A test lives in <strong>one section</strong> but can carry <strong>many tags</strong> —
          that&apos;s the difference. The section is where it files; tags are what it&apos;s{' '}
          <em>for</em>. Tag the fast, critical ones <code>@smoke</code> and you can run just those
          before a merge, then <code>npx playwright test --grep @smoke</code> does the same in CI.
        </p>
      </div>
        ...
        <p className="device-note">
          {deviceId && deviceById(deviceId)?.userAgent ? (
            <>
              ...
              {deviceById(deviceId)?.deviceScaleFactor}× pixel density — so layouts that switch on
              UA or <code>pointer: coarse</code> switch here too.
            ...
            <>
              &ldquo;Size only&rdquo; resizes the window and nothing else — the page still sees a
              desktop browser with no touch. Pick a real device below to test the mobile path
              properly.
            </>
          )}
        </p>
