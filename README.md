...
> **Status: pre-release (v0.1.0) — a working alpha, not yet production-ready.** An independent QA
> audit (Aug 2026) found release blockers; those are fixed and hand-tested, and the remaining work is
> tracked. [CAPABILITIES.md](./CAPABILITIES.md) shows how well each capability is proven, with the
> evidence; [SECURITY.md](./SECURITY.md) covers the security model and known limits.
...
Most test automation starts with writing code. QATestFlow flips that: you drive a real Chrome browser embedded in the app, and it captures what you do as a clean, human-readable list of steps. Those steps become a test you can **replay**, **edit without code**, **save to a library**, and **export to runnable Playwright**. It checks both _behavior_ and _looks_ (visual regression), records a scrubbable **trace** of every run, and **self-heals** broken selectors. When a replay fails, the app explains _why_ — and even tells you whether it looks like an app bug or a test bug.
...
- 🪟 **iframes & Shadow DOM** — records and replays _inside_ embedded frames and open shadow roots (Stripe-style checkouts, web-component UIs).
...
- 🩺 **Failure recovery** — when a replay fails it _pauses_ instead of dying; retry, **continue** (bypass to check later steps), skip the step, or stop.
...
Recording and replay use the browser built into the app — nothing extra to install. Three
features run tests with **real Playwright** instead, and need Playwright's own browsers
downloaded once: **cross-browser runs**, **parallel suite runs**, and **monitors**. The app offers
to download them the first time you use one of those (a "browsers aren't downloaded yet" panel with
a Download button). From a terminal you can do the same with:

```bash
npx playwright install chromium            # enough for parallel runs and monitors
npx playwright install chromium firefox webkit   # for cross-browser runs
```

An exported spec is ordinary Playwright: run it in your own project with `npx playwright test`,
after `npx playwright install` there.
...
4. Click **■ Stop**, then **▶ Replay** to watch it run — behavior _and_ visuals checked together.
...
Signing, the app icon and auto-update are covered in [RELEASING.md](./RELEASING.md).
