          ...
          <div className="modal-backdrop" onClick={() => setSecretMigration(null)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body bundle-body">
                <p>
                  Passwords were still saved in readable form in some of your files. They are now
                  encrypted on this computer, and each file keeps only a reference to them.
                </p>
                {secretMigration.migrated > 0 && (
                  <>
                    <p>
                      <strong>{secretMigration.migrated}</strong> test
                      {secretMigration.migrated === 1 ? '' : 's'} updated: password steps, password
                      columns in data tables, and saved history.
                    </p>
                ...
                <p>
                  <strong>Your tests run in the app exactly as before.</strong> In an exported test,
                  a data table with different passwords per row now reads each one from its own
                  environment variable (<code>PASSWORD_1</code>, <code>PASSWORD_2</code>
          ...
          <div className="modal-backdrop" onClick={() => setBundleResult(null)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-body bundle-body">
                ...
                <p>
                  It’s a plain folder — commit it to git so test changes show up in pull requests,
                ...
                <ul className="bundle-omitted">
                  {bundleResult.manifest.secretsPlaceholdered.length > 0 && (
                    <li>
                      ...
                      {bundleResult.manifest.secretsPlaceholdered.length === 1
                        ? ''
                        : 's'} carry <code>{'{{env:PASSWORD}}'}</code> instead. Safe to commit.
                    </li>
                  )}
                  ...
                  {bundleResult.manifest.visualWithoutBaseline.length > 0 && (
                    <li>
                      ...
                      . A baseline is tied to the screen it was captured on, so a shared one fails
                      elsewhere for no real reason. Their <em>first</em> run creates theirs and
              ...
              <div className="modal-body bundle-body">
                ...
                {importPlan.manifest?.visualWithoutBaseline?.length ? (
                  <p className="import-note">
                    ...
                    , and baselines aren’t shared (they’re tied to the screen they were captured
                    on). Your <strong>first</strong> run creates your own baseline and passes
                    without comparing anything — the second run is the first real check.
                  </p>
                ) : null}
                {importPlan.manifest?.secretsPlaceholdered?.length ? (
                  <p className="import-note">
                    ...
                    . Set <code>PASSWORD</code> in your environment (🌐 Run against → manage) before
                    running them.
                  </p>
                ) : null}
              </div>
