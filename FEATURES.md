...
<!-- counts:start -->
**Automated tests: 793 unit tests** (32 files, `npm test`) and **82 browser tests** (6 files, `npm run test:dom`) — including a gate that generates specs with both exporters and runs them in real Playwright. _These numbers are generated by `node tools/doc-counts.mjs`; CI fails if they drift._
<!-- counts:end -->
<!-- prettier-ignore-end -->
...
- **Not built yet:** YAML/JSON test round-trip, drag-and-drop / scroll / comment step types,
  Projects above suites, a background runner that works with the app closed, a general CLI,
  video recordings, hosted share links, and team features (shared workspace, roles, reviews). See
