...
export interface XbrowserModalProps {
  ...
  xbOpen: boolean
  xbResult: Awaited<ReturnType<typeof window.api.xbrowser.run>> | null
  xbRunning: boolean
  xbSel: Set<string>
}
        ...
        {xbInstalled === false ? (
          <div className="env-list">
            <div className="edge-warn">
              ⚠ Cross-browser needs real Playwright (the embedded engine is Chromium only). Install
              it once, then reopen this:
              <pre className="xb-install">
                npm i -D @playwright/test{'\n'}npx playwright install
              </pre>
              Tip: you can run these right here by typing <code>! npm i -D @playwright/test</code>{' '}
              then <code>! npx playwright install</code>.
            </div>
          ...
          <div className="env-list">
            <div className="edge-warn">
              ...
              <p className="env-list-intro">
                QATestFlow ships the Playwright test runner, but the browser engines it drives
                (Chromium, Firefox, WebKit) are about 400 MB, so they&rsquo;re fetched once on first
                use and shared with any other Playwright project on this machine. Recording and
                in-app replay work without them &mdash; headless runs, parallel suite runs and
                cross-browser need them.
              </p>
            ...
            <div className="modal-footer">
              {xbInstalling ? (
                ...
                <>
                  ...
                  <button className="modal-btn" onClick={() => handleInstallBrowsers(['chromium'])}>
          ...
          <div className="env-edit">
            <p className="env-list-intro">
              Run this test on real browser engines via Playwright. Chromium is what the app already
              uses; Firefox &amp; WebKit catch engine-specific bugs. Session / HAR / upload assets
              aren&rsquo;t included in this run (v1).
            </p>
            ...
            <div className="modal-footer">
              {xbRunning && (
                <span className="edge-count">
                  Running on {xbSel.size} engine{xbSel.size === 1 ? '' : 's'}… (first WebKit run can
                  take a minute)
                </span>
              )}
